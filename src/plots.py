...
def load_stack(path: pathlib.Path) -> np.ndarray:
    if not path.exists():
        print(f"⚠️ Warning: File path {path.name} was not discovered. Skipping entry.")
        return None
...
def generate_temperature_diagnostic_plots() -> None:
    ...
    plt.title(f"Longitudinal Spatial Kinetic Energy Spectrum: {var_name}", fontsize=11, fontweight="bold")
    ...
    vals_min = min(cerra.min(), *(s.min() for s in model_stacks.values()))
    vals_max = max(cerra.max(), *(s.max() for s in model_stacks.values()))
    bins = np.linspace(vals_min, vals_max, n_bins + 1)
    flat_ref = cerra.ravel()
    pdf_ref, edges = np.histogram(flat_ref, bins=bins, density=True)
    centres_ref = 0.5 * (edges[:-1] + edges[1:])
    plt.figure(figsize=(7.5, 5.0))
    plt.plot(centres_ref, np.log10(pdf_ref + eps), lw=3, c="black", label="CERRA (Ground Truth)")
    for name, stack in model_stacks.items():
        pdf_m, _ = np.histogram(stack.ravel(), bins=bins, density=True)
        plt.plot(centres_ref, np.log10(pdf_m + eps), label=name, alpha=0.85, lw=1.5)
    plt.xlabel(f"Temperature values ({var_name} in Kelvin Scale)", fontsize=11)
    plt.ylabel("log10 Probability Density Function (PDF)", fontsize=11)
    plt.title(f"Climatological Extreme Tail Preservation: {var_name}", fontsize=11, fontweight="bold")
...
def print_model_results(low_res, ground_truth, models_list, model_names=None, selected_timesteps=None, datetimes_list=None):
    ...
    for idx_in_list, i in enumerate(selected_timesteps):
        ...
        v_min = min(low_res_c.min(), ground_truth_c.min())
        v_max = max(low_res_c.max(), ground_truth_c.max())
        ...
        for model_arr, name in zip(models_list, model_names):
            ...
            im = ax.imshow(model_c, cmap='viridis', origin='lower', vmin=v_min, vmax=v_max)
            ax.axis('off')
            ax.set_title(f"{time_label}\n{name}")
            col_idx += 1
        ...
        cbar.set_label('Temperature ($T_{2m}$ in $^\circ$C)', fontsize=11, fontweight='bold')
